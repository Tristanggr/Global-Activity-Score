function [S, Score]=Analyse_Donnes(Type)
    filename="C:\Users\antho\Documents\IDS\Sante_Activite_Physique\Scilab\DAT\Anthony1.csv"
    ...
    while(j<=n_epoch)
     ...
     [S,Score]=Calcul_Score(n_Vigorous,n_Moderate,n_Light,Type)

endfunction

function [S,Score]=Calcul_Score(Nb_Temps1,Nb_Temps2,Nb_Temps3,Type)
    if(Type==1)
        S=Nb_Temps1*2+Nb_Temps2+Nb_Temps3*0.5;
     elseif(Type==2)
        S=Nb_Temps1*2+Nb_Temps2+Nb_Temps3*0.125;
    else
        S=Nb_Temps1*2+Nb_Temps2*0.25;
    end
